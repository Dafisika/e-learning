import Navbar from "../../components/navbar/Navbar";
import Sertifikat from "/etc/sertifikat.png";
import Avatar9 from "/avatar/Avatar-9.png";
import Footer from "../../components/footer/Footer";

function Certificate() {
    return (
        <>
            <Navbar account />
            <main className="bg-[#FFFDF3] flex flex-col justify-center py-16 px-30 gap-9">
                <section className="flex gap-1">
                    <a
                        href=""
                        className="font-normal text-base text-[#333333AD] leading-[140%] tracking-[0.2px]"
                    >
                        Beranda <span>/</span>
                    </a>
                    <a
                        href=""
                        className="font-normal text-base text-[#333333AD] leading-[140%] tracking-[0.2px]"
                    >
                        Desain <span>/</span>
                    </a>
                    <a
                        href=""
                        className="font-normal text-base text-[#333333AD] leading-[140%] tracking-[0.2px]"
                    >
                        Gapain Kari.. <span>/</span>
                    </a>
                    <a
                        href=""
                        className="font-normal text-base text-text-dark-primary leading-[140%] tracking-[0.2px]"
                    >
                        Sertifikat
                    </a>
                </section>
                <div className="bg-white flex flex-col border border-[#3A35411F] rounded-[10px] p-6 gap-9">
                    <img src={Sertifikat} alt="" className="rounded-[10px]" />
                    <div className="flex flex-col gap-3">
                        <div className="flex flex-col gap-2">
                            <h1 className="font-semibold text-lg text-text-dark-primary leading-[120%]">
                                Big 4 Auditor Financial Analyst
                            </h1>
                            <p className="font-medium text-base text-[#333333AD] leading-[140%] tracking-[0.2px]">
                                Mulai transformasi dengan instruktur
                                profesional, harga yang terjangkau, dan
                                kurikulum terbaik
                            </p>
                        </div>
                        <div className="flex gap-2.5">
                            <img
                                src={Avatar9}
                                alt=""
                                className="w-10 h-10 rounded-[10px]"
                            />
                            <div className="flex flex-col">
                                <p className="font-medium text-base leading-[140%] tracking-[0.2%] text-text-dark-primary">
                                    Jenna Ortega
                                </p>
                                <p className="font-normal text-sm leading-[140%] tracking-[0.2%] text-[#333333ad]">
                                    Senior Accountant di
                                    <span className="font-bold text-sm leading-[140%] tracking-[0.2%] text-[#333333ad]">
                                        Gojek
                                    </span>
                                </p>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            <div className="flex gap-2">
                                <div className="flex ">
                                    <div className="flex">
                                        <svg
                                            viewBox="0 0 64 64"
                                            xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink"
                                            aria-hidden="true"
                                            role="img"
                                            className="w-[18px] h-[18px]"
                                            preserveAspectRatio="xMidYMid meet"
                                            fill="#000000"
                                        >
                                            <g
                                                id="SVGRepo_bgCarrier"
                                                stroke-width="0"
                                            ></g>
                                            <g
                                                id="SVGRepo_tracerCarrier"
                                                stroke-linecap="round"
                                                stroke-linejoin="round"
                                            ></g>
                                            <g id="SVGRepo_iconCarrier">
                                                <path
                                                    d="M62 25.2H39.1L32 3l-7.1 22.2H2l18.5 13.7l-7 22.1L32 47.3L50.5 61l-7.1-22.2L62 25.2z"
                                                    fill="#ffce31"
                                                ></path>
                                            </g>
                                        </svg>
                                        <svg
                                            viewBox="0 0 64 64"
                                            xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink"
                                            aria-hidden="true"
                                            role="img"
                                            className="w-[18px] h-[18px]"
                                            preserveAspectRatio="xMidYMid meet"
                                            fill="#000000"
                                        >
                                            <g
                                                id="SVGRepo_bgCarrier"
                                                stroke-width="0"
                                            ></g>
                                            <g
                                                id="SVGRepo_tracerCarrier"
                                                stroke-linecap="round"
                                                stroke-linejoin="round"
                                            ></g>
                                            <g id="SVGRepo_iconCarrier">
                                                <path
                                                    d="M62 25.2H39.1L32 3l-7.1 22.2H2l18.5 13.7l-7 22.1L32 47.3L50.5 61l-7.1-22.2L62 25.2z"
                                                    fill="#ffce31"
                                                ></path>
                                            </g>
                                        </svg>
                                        <svg
                                            viewBox="0 0 64 64"
                                            xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink"
                                            aria-hidden="true"
                                            role="img"
                                            className="w-[18px] h-[18px]"
                                            preserveAspectRatio="xMidYMid meet"
                                            fill="#000000"
                                        >
                                            <g
                                                id="SVGRepo_bgCarrier"
                                                stroke-width="0"
                                            ></g>
                                            <g
                                                id="SVGRepo_tracerCarrier"
                                                stroke-linecap="round"
                                                stroke-linejoin="round"
                                            ></g>
                                            <g id="SVGRepo_iconCarrier">
                                                <path
                                                    d="M62 25.2H39.1L32 3l-7.1 22.2H2l18.5 13.7l-7 22.1L32 47.3L50.5 61l-7.1-22.2L62 25.2z"
                                                    fill="#ffce31"
                                                ></path>
                                            </g>
                                        </svg>
                                        <svg
                                            viewBox="0 0 64 64"
                                            xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink"
                                            aria-hidden="true"
                                            role="img"
                                            className="w-[18px] h-[18px]"
                                            preserveAspectRatio="xMidYMid meet"
                                            fill="#c4c4c4"
                                        >
                                            <g
                                                id="SVGRepo_bgCarrier"
                                                stroke-width="0"
                                            ></g>
                                            <g
                                                id="SVGRepo_tracerCarrier"
                                                stroke-linecap="round"
                                                stroke-linejoin="round"
                                            ></g>
                                            <g id="SVGRepo_iconCarrier">
                                                <path
                                                    d="M62 25.2H39.1L32 3l-7.1 22.2H2l18.5 13.7l-7 22.1L32 47.3L50.5 61l-7.1-22.2L62 25.2z"
                                                    fill="#c4c4c4"
                                                ></path>
                                            </g>
                                        </svg>
                                        <svg
                                            viewBox="0 0 64 64"
                                            xmlns="http://www.w3.org/2000/svg"
                                            xmlns:xlink="http://www.w3.org/1999/xlink"
                                            aria-hidden="true"
                                            role="img"
                                            className="w-[18px] h-[18px]"
                                            preserveAspectRatio="xMidYMid meet"
                                            fill="#c4c4c4"
                                        >
                                            <g
                                                id="SVGRepo_bgCarrier"
                                                stroke-width="0"
                                            ></g>
                                            <g
                                                id="SVGRepo_tracerCarrier"
                                                stroke-linecap="round"
                                                stroke-linejoin="round"
                                            ></g>
                                            <g id="SVGRepo_iconCarrier">
                                                <path
                                                    d="M62 25.2H39.1L32 3l-7.1 22.2H2l18.5 13.7l-7 22.1L32 47.3L50.5 61l-7.1-22.2L62 25.2z"
                                                    fill="#c4c4c4"
                                                ></path>
                                            </g>
                                        </svg>
                                    </div>
                                </div>

                                <div className="flex font-medium text-sm leading-[140%] tracking-[0.2%] text-[#333333ad] underline decoration-solid">
                                    <p>3.5 (86)</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </main>
            <Footer />
        </>
    );
}
export default Certificate;
